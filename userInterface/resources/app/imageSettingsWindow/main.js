...
class LoadSavedSettings {
    constructor() {
        this.inputElement = document.getElementById("image-presets")
    }
    ...
    readJSONdata(data) {
        ...
        reader.onload = () => {
            let convertedSaveData = JSON.parse(reader.result)

            collectionOfSliders.getSliderInstances().forEach((sliderInstance) => {
                console.log("reader result", convertedSaveData)
                console.log("convertedSaveData.binarizedValue", convertedSaveData["binarizedValue"])

                const id = sliderInstance.getId()
                switch (id) {
                    case SLIDER_IDS.VALUE_MAX:
                        sliderInstance.updateValue(convertedSaveData.valueMaxValue)
                        break
                    case SLIDER_IDS.HUE_MAX:
                        sliderInstance.updateValue(convertedSaveData.hueMaxValue)
                        break
                    case SLIDER_IDS.SATURATION_MAX:
                        sliderInstance.updateValue(convertedSaveData.saturationMaxValue)
                        break
                    case SLIDER_IDS.VALUE_MIN:
                        sliderInstance.updateValue(convertedSaveData.valueMinValue)
                        break
                    case SLIDER_IDS.HUE_MIN:
                        sliderInstance.updateValue(convertedSaveData.hueMinValue)
                        break
                    case SLIDER_IDS.SATURATION_MIN:
                        sliderInstance.updateValue(convertedSaveData.saturationMinValue)
                        break
                    case SLIDER_IDS.BINARIZED:
                        sliderInstance.updateValue(convertedSaveData.binarizedValue)
                        break
                    default:
...
let loadSavedSettings = new LoadSavedSettings()

loadSavedSettings.listenToTranslationFileUploadEvent()


class SaveSettingsButton {
    constructor() {
        this.button = document.getElementById("saveSettingsButton")
    }
    ...
    saveDataAsJSON(){
        let JSONsaveData =  JSON.stringify(editedImage.returnImageColorValues())
        let saveDataFileName = "colorSettingsData"
...
let saveSettingsButton = new SaveSettingsButton()

saveSettingsButton.listenToClickEvent()
